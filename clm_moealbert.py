...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        if attention_mask is not None:
            ...
            final_mask = torch.logical_or(causal_mask, padding_mask)
        else:
            final_mask = causal_mask
            
        mask_value = torch.finfo(scores.dtype).min
        scores = scores.masked_fill(final_mask, mask_value)
        # --- End Causal Masking ---
...
class AlbertForCausalLM(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, labels=None):
        ...
        if labels is not None:
            ...
            total_loss = clm_loss + self.config.aux_loss_coefficient * aux_loss
            loss = total_loss
...
def main():
    ...
    eval_group.add_argument("--eval_tasks", type=str, default="basic", choices=["basic", "comprehensive", "custom"])
    ...
    if args.mode in ["pretrain", "all"]:
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch in progress_bar:
                ...
                progress_bar.set_postfix(total_loss=loss.item(), clm_loss=clm_loss.item(), aux_loss=aux_loss.item(), l1_penalty=log_data['train/l1_penalty'])
    ...
    if args.mode in ["evaluate", "all"]:
        ...
        if args.eval_tasks == "basic":
            task_list = ["STSBenchmark"]
        elif args.eval_tasks == "comprehensive":
            task_list = [t.metadata.name for t in get_tasks()]
        else:
            task_list = args.custom_tasks
